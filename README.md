...
Usage Overlay is an unofficial Windows companion that keeps your remaining Codex quota visible beside the desktop app. It runs as a separate process, uses the documented Codex App Server protocol, and never injects code into Codex.
...
- Supports Follow Codex, Dark, and Light themes. Follow Codex matches the active Codex app and falls back to Windows when Codex is unavailable.
...
The native Settings window uses explicit Save and Cancel controls. Theme previews and safe display changes apply immediately; CLI path and refresh interval changes are saved for the next restart. Choose Follow Codex, Dark, or Light under Appearance.
...
Usage Overlay is an independent open-source project and is not affiliated with or endorsed by OpenAI. Codex and OpenAI are referenced only to describe compatibility.
